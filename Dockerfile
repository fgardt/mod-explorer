...
ENV LEPTOS_OUTPUT_NAME=mod_explorer \
...
# Frontend deps
RUN cargo chef cook --no-default-features --features hydrate --profile wasm-release --target wasm32-unknown-unknown --package mod-explorer --target-dir target/front
# Backend deps
RUN cargo chef cook --no-default-features --features ssr --release --package mod-explorer --bin mod-explorer
